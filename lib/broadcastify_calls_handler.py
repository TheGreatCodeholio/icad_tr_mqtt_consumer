...
module_logger = logging.getLogger('icad_tr_consumer.broadcastify_calls')
...
def upload_to_broadcastify_calls(broadcastify_config, temp_path, call_data):
    ...
    try:
        m4a_path = os.path.join(temp_path, f'{call_data.get("filename").replace(".wav", ".m4a")}')
        ...
        with open(m4a_path, 'rb') as audio_file:
            files = {
                'metadata': (os.path.basename(m4a_path.replace("m4a", ".json")), json_bytes, 'application/json'),
                'audio': (os.path.basename(m4a_path), audio_file, 'audio/aac'),
